<?php    
    ...
    if( sizeof($_POST) > 0 && isset($_POST['accion']) && ($_POST['accion'] == 'NUEVO') ){
		...
		$grupo = $_POST["grupo"];
		if (empty($grupo)){
			$sqlQuery = "INSERT INTO `socios` (`id_socio`, `dni`, `nombre`, `apellido`, `direccion`, `telefono`, `mail`, `fecha_nac`, `habilitado`, `tipo_dni`) 
			VALUES (NULL, '".$dni."', '".$nombre."', '".$apellido."', '".$dir."', '".$tel."', '".$mail."', '".$fec_nac."', '".$habilitado."', '".$tdni."')";
		}else{
			$sqlQuery = "INSERT INTO `socios` (`id_socio`, `dni`, `nombre`, `apellido`, `direccion`, `telefono`, `mail`, `fecha_nac`, `habilitado`, `tipo_dni`, `id_grupo`) 
...
<html class="no-js" lang="">
    <head>
        ...
        <title>Clase 08 - MySQL - AMBC</title>
